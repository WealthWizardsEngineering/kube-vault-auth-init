...
echo "Get .auth.client_token"
VAULT_LOGIN_TOKEN=$(curl -k -sS --request POST \
...
VAULT_ROLE_ID=$(curl -k -sS --header "X-Vault-Token: ${VAULT_LOGIN_TOKEN}" \
...
VAULT_SECRET_ID=$(curl -k -sS --header "X-Vault-Token: ${VAULT_LOGIN_TOKEN}" \
  --request POST \
...
APPROLE_TOKEN=$(curl -k -sS --request POST \
...
#########################################################################
## Commented this section out, as Consul-Template will do all this lifting for us
############
...
#
#  LOOKUP_SECRET_RESPONSE=$(curl -k -sS \
